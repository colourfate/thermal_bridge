...
# 8 "../Core/Src/mlx90640_adapter.c" 2
...
# 80 "D:/Workspace/stm32/ThermalBridge3/Hardware/headers/MLX90640_API.h"
typedef struct
    {
        int16_t kVdd;
        int16_t vdd25;
...
# 10 "../Core/Src/mlx90640_adapter.c" 2
...
int MLX90640_I2CRead(uint8_t slaveAddr, uint16_t startAddress, uint16_t nMemAddressRead, uint16_t *data)
{
 ...
 ret = HAL_I2C_Mem_Read(&hi2c1, (slaveAddr << 1), startAddress, 0x00000010U, p, nMemAddressRead * 2, 1000);
 if (ret != HAL_OK) {
  printf("%s(%d): " "HAL_I2C_Mem_Read failed\r\n", __func__, 34);
  return HAL_ERROR;
 }
...
int MLX90640_I2CWrite(uint8_t slaveAddr,uint16_t writeAddress, uint16_t data)
{
 ...
 ret = HAL_I2C_Mem_Write(&hi2c1, (slaveAddr << 1), writeAddress, 0x00000010U, cmd, sizeof(cmd), 1000);
 if (ret != HAL_OK) {
  printf("%s(%d): " "HAL_I2C_Mem_Write failed\r\n", __func__, 58);
  return HAL_ERROR;
 }
...
void mlx_param_init(void)
{
 ...
 status = MLX90640_SetRefreshRate(0x33, FPS16HZ);
 if (status != 0) {
  printf("%s(%d): " "Set refresh rate failed\r\n", __func__, 75);
  return;
 }

 status = MLX90640_SetChessMode(0x33);
 if (status != 0) {
  printf("%s(%d): " "Set chess mode failed\r\n", __func__, 81);
  return;
 }

 status = MLX90640_DumpEE(0x33, g_eeMLX90640);
 if (status != 0) {
  printf("%s(%d): " "Failed to load system parameters\r\n", __func__, 87);
  return;
 }
 ...
 if (status != 0) {
  printf("%s(%d): " "Parameter extraction failed: %d\r\n", __func__, 99, status);
  return;
 }

 printf("%s(%d): " "kVdd: %d\r\n", __func__, 103, g_mlx90640_param.kVdd);
 printf("%s(%d): " "vdd25: %d\r\n", __func__, 104, g_mlx90640_param.vdd25);
 printf("%s(%d): " "KvPTAT: %f\r\n", __func__, 105, g_mlx90640_param.KvPTAT);
 printf("%s(%d): " "KtPTAT: %f\r\n", __func__, 106, g_mlx90640_param.KtPTAT);
 printf("%s(%d): " "vPTAT25: %d\r\n", __func__, 107, g_mlx90640_param.vPTAT25);
}
...
void mlx_data_read(float data[768])
{
 ...
 for (int x = 0 ; x < 2 ; x++) {
  ...
  status = MLX90640_GetFrameData(0x33, g_mlx90640Frame);
  if (status < 0) {
   printf("%s(%d): " "GetFrame Error: %d\r\n", __func__, 122, status);
  }
